...
import {filter, map, reduce, switchMap} from "rxjs/internal/operators";
...
export class UserService {
  ...
  searchUsers(str: string): Observable<User[]> {
    ...
    return this.http.get(uri, {params: {'name_like': str}}).pipe(map(res => res as User[]));
  }
  ...
  getUsersByProject(projectId: string): Observable<User[]> {
    ...
    return this.http.get(uri, {params: {projectId}}).pipe(map(res => res as User[]));
  }
  ...
  // 把某项目分配给某用户
  addProjectRef(user: any, projectId: string): Observable<User> {
    const uri = `${this.config.uri}/${this.domain}/${user.id}`;
    const projectIds = (user.projectIds) ? user.projectIds : [];
    if (projectIds.indexOf(projectId) !== -1) return of(user);
    return this.http.patch(uri, JSON.stringify({projectIds: [...projectIds, projectId]}), {headers: this.headers}).pipe(map(res => res as User));
  }
}
